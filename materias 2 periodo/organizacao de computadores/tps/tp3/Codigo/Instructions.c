...
//Função acrescentada para o tp3
void create_hardDisk_bin(FILE* create, int blocks){
  int word0=0, word1=0 , word2=0, word3=0;
  if(create != NULL){
    for(int i = 0;i<blocks; i++){
      ...
      word2 = rand()%1000;
      word3 = rand()%1000;
      fwrite(&word0,sizeof(int),1,create);
      fwrite(&word1,sizeof(int),1,create);
      fwrite(&word2,sizeof(int),1,create);
      fwrite(&word3,sizeof(int),1,create);
    }
    rewind(create);
  }
}
